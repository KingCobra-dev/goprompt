...
export function CreatePackPage({ onBack, onPackCreated }: CreatePackPageProps) {
  ...
  const userPrompts = state.prompts.filter(
    p => p.userId === state.user?.id && p.visibility === 'public'
  )
  ...
  const validateForm = (): boolean => {
    ...
    if (!name.trim()) newErrors.name = 'Pack name is required'
      ...
      newErrors.name = 'Pack name must be less than 60 characters'
    ...
    if (selectedPrompts.length < 3)
      newErrors.prompts = 'At least 3 prompts are required for a pack'
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-4">
            Please sign in to create prompt packs.
          </p>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm text-muted-foreground">
                  ...
                  <span
                    className={description.length > 270 ? 'text-warning' : ''}
                  >
            ...
            <CardContent>
              {userPrompts.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  You need to create public prompts first before creating a
                  pack.
                </p>
              ) : (
