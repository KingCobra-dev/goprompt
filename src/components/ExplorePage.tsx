...
export function ExplorePage({
  ...
  useEffect(() => {
    const loadPrompts = async () => {
      try {
        ...
        const { data: publicPrompts, error: publicError } =
          await promptsApi.getAll()
        console.log('ExplorePage: promptsApi.getAll() returned:', {
          dataLength: publicPrompts?.length,
          error: publicError,
        })
        ...
        if (state.user) {
          console.log(
            'ExplorePage: User logged in, loading user prompts for user:',
            state.user.id
          )
          const { data: userPromptsData, error: userError } =
            await promptsApi.getByUser(state.user.id)
          console.log('ExplorePage: promptsApi.getByUser() returned:', {
            dataLength: userPromptsData?.length,
            error: userError,
          })
        ...
        console.log(
          'ExplorePage: Combining prompts - public:',
        ...
        console.log(
          'ExplorePage: All prompts before deduplication:',
          allPrompts.length
        )
        ...
        console.log(
          'ExplorePage: Unique prompts after deduplication:',
          uniquePrompts.length
        )

        if (publicError) {
          console.error(
            'ExplorePage: Error loading public prompts:',
            publicError
          )
          ...
          console.log(
            'ExplorePage: Loaded',
            uniquePrompts.length,
            'unique prompts from database'
          )
          console.log('ExplorePage: Starting prompt transformation')

          // Transform database prompts to match our Prompt type
          const transformedPrompts: Prompt[] = uniquePrompts.map(
            (dbPrompt: any) => {
              console.log(
                'ExplorePage: Transforming prompt:',
                dbPrompt.id,
                dbPrompt.title
              )
              return {
                ...
                author: dbPrompt.profiles
                  ? {
                      ...
                      subscriptionStatus:
                        dbPrompt.profiles.subscription_status || 'active',
                ...
                images:
                  dbPrompt.prompt_images?.map((img: any) => ({
                    id: img.id,
                    url: img.url,
                ...
                isHearted: state.user
                  ? state.hearts.some(
                      h =>
                        h.userId === state.user!.id &&
                        h.promptId === dbPrompt.id
                    )
                  : false,
                isSaved: state.user
                  ? state.saves.some(
  ...
  useEffect(() => {
    if (initialSearchQuery) {
      dispatch({
        ...
        payload: { query: initialSearchQuery },
      })
    }
  ...
  const filteredPrompts = useMemo(() => {
    ...
    if (filters.types.length > 0) {
      filtered = filtered.filter(prompt => filters.types.includes(prompt.type))
    }

    // Model filters
    if (filters.models.length > 0) {
      filtered = filtered.filter(prompt =>
        prompt.modelCompatibility.some(model => filters.models.includes(model))
      )
    }
    ...
    if (filters.successRateMin !== undefined) {
      filtered = filtered.filter(
        prompt => (prompt.successRate || 0) >= filters.successRateMin!
      )
    }

    if (filters.successRateMax !== undefined) {
      filtered = filtered.filter(
        prompt => (prompt.successRate || 0) <= filters.successRateMax!
      )
    }
      ...
      switch (filters.sortBy) {
        case 'trending': {
          ...
          const getScore = (prompt: Prompt) => {
            const score =
              prompt.hearts + prompt.saveCount * 2 + prompt.forkCount * 3
            const ageInDays =
              (Date.now() - new Date(prompt.createdAt).getTime()) /
              (1000 * 60 * 60 * 24)
                        ...
                        <Badge
                          ...
                          onClick={() => {
                            ...
                            if (filters.types.includes(filter as string)) {
                              ...
                              filters.models.includes(filter as string)
                            ) {
                              const model = memoizedModels.find(
                                m => m.name === filter
                              )
                              if (model) handleModelToggle(model.id)
                            } else if (
                              filters.tags.includes(filter as string)
                            ) {
                              updateFilters({
                                tags: filters.tags.filter(t => t !== filter),
                              })
                            } else if (
                              filters.categories.includes(filter as string)
                            ) {
                              const category = memoizedCategories.find(
                                c => c.name === filter
                              )
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      ...
                      <Select
                        value={
                          filters.successRateMin !== undefined
                            ? filters.successRateMin.toString()
                            : 'any'
                        }
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Select
                        value={
                          filters.successRateMax !== undefined
                            ? filters.successRateMax.toString()
                            : 'any'
                        }
                        onValueChange={(value: string) =>
                          updateFilters({
              ...
              {filteredPrompts.map(prompt => (
                <PromptCard
                  ...
                  onSave={async () => {
                    ...
                    const isSaved = state.saves.some(
                      s =>
                        s.userId === state.user!.id && s.promptId === prompt.id
                    )

                    if (!isSaved) {
                      const userSaves = state.saves.filter(
                        s => s.userId === state.user!.id
                      )
                      const { allowed, message } = canSaveMore(
                        state.user,
                        userSaves.length
                      )
