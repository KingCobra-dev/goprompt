...
export const PromptTypeSelector: React.FC<PromptTypeSelectorProps> = ({
  ...
  const [selectedValue, setSelectedValue] = useState<PromptType | undefined>(
    value
  )
  const [focusedIndex, setFocusedIndex] = useState<number>(-1)

  const handleSelect = useCallback(
    (option: PromptTypeOption) => {
    ...
    (event: React.KeyboardEvent, index: number) => {
      switch (event.key) {
        ...
        case 'ArrowLeft':
          ...
          setFocusedIndex(
            prev =>
              (prev - 1 + promptTypeOptions.length) % promptTypeOptions.length
          )
          break
        case 'ArrowDown':
          event.preventDefault()
          setFocusedIndex(prev =>
            Math.min(prev + 2, promptTypeOptions.length - 1)
          )
          break
        case 'ArrowUp':
          event.preventDefault()
