...
export function UserManagement() {
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      const { data: _data, error, count } = await adminAPI.getUsers(filters)
  ...
  const handleSaveUser = async () => {
    ...
    try {
      ...
      const { data: _data2, error } = await adminAPI.updateUserProfile(
        selectedUser.id,
        updates
      )
  ...
  const handleRoleChange = async (
    userId: string,
    newRole: 'general' | 'pro' | 'admin'
  ) => {
    try {
      const { data: _data3, error } = await adminAPI.updateUserRole(
        userId,
        newRole
      )
  ...
  const handleUserAction = (
    user: UserProfile,
    action: 'ban' | 'disable' | 'enable' | 'unban'
  ) => {
    setSelectedUser(user)
    ...
    try {
      ...
      switch (actionType) {
        ...
        case 'unban':
          ...
          break
        case 'disable':
          result = await adminAPI.disableUser(
            selectedUser.id,
            actionReason || undefined
          )
          break
      ...
      <Dialog open={actionDialogOpen} onOpenChange={setActionDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {actionType === 'ban' &&
                'This will ban the user from accessing the platform.'}
              {actionType === 'disable' &&
                'This will disable the user account temporarily.'}
              {actionType === 'unban' &&
                'This will restore access to the user account.'}
              {actionType === 'enable' &&
                'This will re-enable the user account.'}
            </DialogDescription>
          </DialogHeader>
