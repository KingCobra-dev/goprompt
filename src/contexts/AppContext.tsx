...
type AppAction =
  ...
  | {
      type: 'SAVE_PROMPT'
      payload: { promptId: string; collectionId?: string }
    }
  ...
  | {
      ...
      payload: { id: string; updates: Partial<Portfolio> }
    }
  ...
  | {
      ...
      payload: { packId: string; packName: string }
    }
...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case 'SAVE_PROMPT': {
      ...
      const existingSave = state.saves.find(
      ...
      const updatedPrompts = state.prompts.map(p =>
        p.id === promptId
          ? { ...p, saveCount: p.saveCount + 1, isSaved: true }
          : p
      )
    ...
    case 'UNSAVE_PROMPT': {
      ...
      const updatedSaves = state.saves.filter(
        s => !(s.userId === state.user!.id && s.promptId === promptId)
      )

      const updatedPrompts = state.prompts.map(p =>
        p.id === promptId
          ? { ...p, saveCount: Math.max(0, p.saveCount - 1), isSaved: false }
          : p
      )
    ...
    case 'FORK_PROMPT': {
      ...
      if (
        originalPrompt &&
        originalPrompt.userId !== newPrompt.userId &&
        state.user
      ) {
        const notification: Notification = {
          id: `notification-${Date.now()}`,
    ...
    case 'FOLLOW_USER': {
      ...
      const existingFollow = state.follows.find(
        f => f.followerId === state.user!.id && f.followingId === followingId
      )
      ...
      const updatedDrafts =
        existingDraftIndex >= 0
          ? state.drafts.map((d, i) =>
              i === existingDraftIndex ? action.payload : d
            )
    ...
    case 'ADD_PACK_TO_LIBRARY': {
      ...
      const currentLibrary = state.userPackLibrary || {
        userId: state.user.id,
        packs: [],
      }
        ...
        updatedLibrary = {
          ...
          packs: updatedLibrary.packs.map(pack =>
            pack.packId === packId
              ? {
                  ...pack,
                  addedToPortfolios: [...pack.addedToPortfolios, portfolioId],
                }
              : pack
          ),
        }
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadUserProfile = async (authUser: any) => {
    try {
      console.log('[loadUserProfile] Starting for authUser:', {
        id: authUser.id,
      ...
      if (authUser.email && ADMIN_EMAILS.includes(authUser.email)) {
        console.log(
          '[loadUserProfile] Admin bypass activated for email:',
          authUser.email
        )
      ...
      if (profile && !error) {
        console.log('[loadUserProfile] Existing profile found:', {
          role: profile.role,
          email: profile.email,
        ...
        console.log(
          '[loadUserProfile] Subscription status:',
        ...
        if (user.email && ADMIN_EMAILS.includes(user.email)) {
          ...
          user.role = 'pro'
          console.log(
            '[loadUserProfile] Setting role to pro due to active subscription'
          )
        } else {
          user.role = 'general'
        ...
        if (fetchedPrompts && !promptsError) {
          ...
          console.error(
            '[loadUserProfile] Error fetching prompts:',
            promptsError
          )
        }
      } else {
        console.log(
          '[loadUserProfile] No existing profile found, creating new one'
        )
        ...
        console.log(
          '[loadUserProfile] New profile subscription status:',
          subscriptionStatus
        )
        ...
        if (createdProfile && !createError) {
          ...
          console.log(
            '[loadUserProfile] New profile created with final role:',
            user.role
          )
          ...
          async (payload: any) => {
            try {
              if (payload.eventType === 'INSERT') {
                ...
                const { data: newPrompt } = await promptsAPI.getById(
                  payload.new.id
                )
                if (newPrompt) {
                  // Transform the data to match Prompt interface (user_id -> author)
                ...
                const { data: updatedPrompt } = await promptsAPI.getById(
                  payload.new.id
                )
                if (updatedPrompt) {
                  // Transform the data to match Prompt interface (user_id -> author)
